	...
	)
{
	...
	rstrm->the_buffer = malloc(sendsize + recvsize + BYTES_PER_XDR_UNIT);
...
xdrrec_getlong(XDR *xdrs,long *lp)
{
	...
	long *buflp = (long *)(rstrm->in_finger);
	long mylong;
...
{
	RECSTREAM *rstrm = (RECSTREAM *)(xdrs->x_private);
	long *dest_lp = ((long *)(rstrm->out_finger));
...
xdrrec_destroy(XDR *xdrs)
{
	...
	free(rstrm->the_buffer);//,		rstrm->sendsize + rstrm->recvsize + BYTES_PER_XDR_UNIT);
	free((caddr_t)rstrm);//, sizeof(RECSTREAM));
}
...
flush_out(RECSTREAM *rstrm, bool_t eor)
{
	...
	u_long len = (u_long)(rstrm->out_finger) -
