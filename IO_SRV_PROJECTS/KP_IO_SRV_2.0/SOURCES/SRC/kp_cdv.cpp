...
bool KpCommDev::Reset(const char *pt_name,int spd)
{
  ...
  {
	  dump.Print(D_ERR | D_PORT,"\tПорт %s не доступен!", port_name);    
    return ok=false;
  }
  ...
  {
    dump.Print(D_ERR | D_PORT,"\tОшибка установки параметров порта %s !", port_name);
    return ok=false;
  }
...
#endif
  dump.Print(D_MSG | D_PORT,"\tПорт %s открыт...", port_name);
  return (ok=true);  
}
//---------------------------------------------------------------
...
void KpCommDev::Process()
{
  if(!Ok())
    return ;
  int sz;
  unsigned char clear_buff[512]; //22.04.02
    ...
    if(head->address > 3) //пакет канального уровня
    {
      switch(data[3])     //тип пакета
      {
        case 0xa0:
          ...
          InitChannels();        
          while(ReadPort(clear_buff,512,sz) > 0){}//22.04.02
          break;
        case 0xb0:
...
//-------------------------------------------------------------------
void KpCommDev::SendStep()//MEN 17.04.02
{
  int sended;
  list<BinData *>::iterator i;
  ...
  if(OutBuff.Len)
  {
    ...
    if(sended == -1)
    {
      if(errno == EIO) //ошибка ввода - вывода
      {
        char *name = strdup(port_name);
        dump.ObjMsg(cfg_d.Id(),D_ERR | D_PORT,"Порт %s(%s): ошибка ввода-вывода! Реинициализация...",cfg_d.Name(),Name());
        Reset(name,speed);//открытие-закрытие порта
        InitChannels();
        free(name);
      }
    }
    else if(sended != OutBuff.Len)
    {
